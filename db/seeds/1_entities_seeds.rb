...
Entity.find_or_create_by! code: 'neu_percent', title: 'Hейтрофилы %', alias: 'Neu%', group_id: blood_group.id,
                          min: 40, max: 78, unit: '%',
                          description: 'Hейтрофилы генерируются в костном мозге. Срок их службы в крови длится несколько часов. Нейтрофилы уничтожают микробы (фагоцитоз).'
Entity.find_or_create_by! code: 'neu_rod_shaped_percent', title: 'Палочкоядерный нейтрофилы %', group_id: blood_group.id,
                          min: 1, max: 6, unit: '%',
                          description: 'Палочкоядерный нейтрофил — юная разновидность нейтрофилов. У него S-образное ядро (в отличие от сегментоядерного). Палочкоядерные нейтрофилы представляют собой один из последних этапов дифференцировки нейтрофильного ряда гранулоцитов (зернистых лейкоцитов) в сегментоядерные нейтрофилы. В крови содержится только 1–6 % палочкоядерных нейтрофилов из общего числа лейкоцитов. Наличие палочкоядерных нейтрофилов в русле крови объясняется тем, что они завершают миелопоэз непосредственно в сосудах. Палочкоядерные нейтрофилы присутствуют в крови непродолжительно. У них быстро сегментируется ядро (происходит дозревание), и они превращаются в сегментоядерные. Клиническое значение имеет так называемый «сдвиг (смещение) лейкоцитарной формулы вправо или влево». Под сдвигом понимают изменение соотношения сегментоядерных и палочкоядерных (зрелых и юных) нейтрофилов.'
...
Entity.find_or_create_by! code: 'prothrombin_time', title: 'Протромбиновое время',
                          min: 10, max: 13.2, unit: 'сек',
                          description: 'Промежуток времени, за который образуется кровяной сгусток, что позволяет оценить работу свёртывающей системы крови. Часто исследование назначают при плановых обследованиях беременных, перед хирургическим вмешательством и для оценки эффективности терапии антикоагулянтными препаратами.'
Entity.find_or_create_by! code: 'prothrombin', title: 'Протромбин (по Квику)',
                          min: 10, max: 80, unit: '%',
                          description: 'Протромбин по Квику и международное нормализованное отношение (МНО) – это показатели протромбинового времени, с помощью которых оценивается свертывающая способность крови. При кровотечении активируется система коагуляции, которая закупоривает кровеносный сосуд тромбом. Конечным результатом является образование нитей фибрина, которые соединяются в месте повреждения с агрегированными клеточными фрагментами, называемыми тромбоцитами, с образованием стабильного сгустка крови. Гемостаз включает в себя серию последовательных химических реакций, называемых каскадом коагуляции, в которых факторы коагуляции (или свертывания) активируются один за другим и приводят к образованию сгустка. Слишком малое количество факторов свертывания может привести к чрезмерному кровотечению; слишком большое – к чрезмерному свертыванию. Тест на протромбиновое время и МНО позволяет определить, насколько нарушена система свертываемости крови, а также оценить эффективность лечения антикоагулянтами, предотвращающими образование тромбов.'
Entity.find_or_create_by! code: 'inr', title: 'Международное нормализованное отношение (МНО)',
                          min: 0.8, max: 1, unit: '%',
                          description: 'Международное нормализованное отношение (МНО) – это лабораторное измерение того, сколько времени требуется крови для образования сгустка. Используется для оценки степени влияния антикоагулянтов на свертывающие способности крови. Измерение протромбинового времени имеет свои преимущества – простота и доступная стоимость.'
Entity.find_or_create_by! code: 'aptt', title: 'АЧТВ', alias: 'АПТВ, активированное частичное (парциальное) тромбопластиновое время, кефалин, каолиновое время',
                          min: 25.4, max: 36.9, unit: 'сек',
                          description: 'Активированное частичное тромбопластиновое время (АЧТВ) позволяет оценить общую сохранность функций факторов свертывания, выявить дефицит или нарушение функций некоторых факторов свертывания и контактных факторов внутреннего (VIII, IX, XI, XII, прекалликреина, высокомолекулярных кининогенов) и общего пути свертывания (включая факторы II, V, X и I). Этот тест получил свое название от названий реагентов (парциальные тромбопластины), которые добавляют для запуска реакции. Этим исследованием определяют время образования сгустка после добавления каолин-кефалиновой смеси и CaCl2 к бестромбоцитарной цитратной плазме. Реагент для АЧТВ теста содержит контактный активатор (суспензия каолина) и фосфолипиды (кефалин). Контакт плазмы с частицами каолина стимулирует производство активного фактора XII-XIIa, предоставляя поверхность для функционирования высокомолекулярного кининогена, калликреина и фактора XIIa. Фосфолипиды необходимы для образования комплексов с активным фактором X (Xa) и протромбином. После определенного времени инкубации в реакционную смесь добавляется хлорид кальция. Тем самым имитируется запуск свертывания по внутреннему пути и выявляется возможный дефицит факторов, участвующих в нем, или наличие ингибиторов свертывания.'
Entity.find_or_create_by! code: 'fibrinogen', title: 'Фибриноген',
                          min: 2, max: 4, unit: 'г/л',
                          description: 'Фибриноген – белок, предшественник фибрина, составляющего основу сгустка при свертывании крови. Исследование направлено на оценку способности организма к тромбообразованию и выявлению связанных с этим процессом нарушений.'
Entity.find_or_create_by! code: 'clotting_time', title: 'Свертываемость крови по Сухареву',
                          min: 30, max: 300, unit: 'сек',
                          description: 'Процесс свертывания показывает, насколько организм может защищать сам себя, в том числе от потери крови. Свертываемость напрямую зависит от функционирования эндокринной, а также нервной системы. Для определения состояния и оценки гемостаза (общей системы свертываемости) назначается коагулограмма, а так же время свертывания крови (по Сухареву). По сути метод Сухарева позволяет оценить интервал между забором крови и началом ее сгущения, точнее появление в ней фибрина (сгустка).'
Entity.find_or_create_by! code: 'bleeding_time', title: 'Время кровотечения по Дуке',
                          min: 30, max: 180, unit: 'сек',
                          description: 'Длительность кровотечения - это оценка состояния тромбоцитов. Если сосудистые стенки повреждены, то в идеале тромбоциты должны немедленно активизироваться, чтобы принять участие в свертывании - гемокоагуляции. Сосуды при нормальной эластичности спазмируются и закупориваются агрегированными тромбоцитами (агрегация – склеивание, слипание клеток). Таким образом, сосуд закрывается первичным, очень важным образованием, который называют «белый тромб».'
